...
line_attr_fields = ['LineType', 'MaterialIndex', 'StartCrossSectionIndex',
                    'EndCrossSectionIndex', 'AutoEccentricityType']


def get_line_attributes(obj, *args, i=None, fields=None, raw=False,
    ...
    def xyz2(p): return [p.xx, p.yy, p.zz]
    
    data = {}
    
    if 'LineType' in fields:
        data['LineType'] = list(
            map(lambda r: linetype_to_str[r.LineType], rec))
    ...
    if 'EndCrossSectionIndex' in fields:
        data['EndCrossSectionIndex'] = list(
            map(lambda r: r.EndCrossSectionIndex, rec))
    if 'AutoEccentricityType' in fields:
        data['AutoEccentricityType'] = list(
            map(lambda r: r.AutoEccentricityType, rec))
    ...
    if 'SpringDirection' in fields:
        data['SpringDirection'] = list(map(lambda r: springdir_to_str[r.SpringDirection], rec))
...
class IAxisVMLines(AxisVMModelItems):
    ...
    def get_line_data(self, *args, i=None, fields=None, raw=False) -> AxisVMAttributes:
        ...
        if 'GeomType' in fields:
            data['GeomType'] = list(
                map(lambda r: geomtype_to_str[r.GeomType], rec))
        if 'CircleArc' in fields:
            def xyz(p): return [p.x, p.y, p.z]

            def foo(ca): return dict(
                Center=xyz(ca.Center),
                NormalVector=xyz(ca.NormalVector),
                Alfa=ca.Alfa
            )
            def CircleArc(r): return foo(
                r.CircleArc) if r.GeomType == 1 else None
    ...
    def get_attributes(self, *args, fields=None, **kwargs) -> AxisVMAttributes:
        ...
        res = {}
        if len(afields) > 0:
            res.update(self.get_line_attributes(
                *args, fields=afields, **kwargs))
        if len(dfields) > 0:
